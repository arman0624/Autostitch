...
class TestWarp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.img_bl = warp.warpSpherical(blank,
                                         parameters[0],
                                         parameters[1],
                                         parameters[2])
        self.org_bl = resBl
    ...
    def test_computeSphericalWarpMappings(self):
        ...
        self.assertTrue(np.allclose(self.img_bl,
                                    self.org_bl,
                                    rtol=1e-05,
                                    atol=1e-05), 'Error in Spherical warping')
...
class TestAlignment(unittest.TestCase):
    ...
    def test_computehomography3(self):
        ...
        self.assertTrue(np.allclose(np.eye(3),
                                    H_student,
                                    rtol=1e-05,
                                    atol=1e-05),
    ...
    def test_alignPair(self):
        M = alignment.alignPair(self.f1,
                                self.f2,
                                self.matches,
                                alignment.eHomography,
                                1,
                                1)

    def test_getInliers(self):
        inliers = alignment.getInliers(self.f1+[self.outlier_f1],
                                       self.f2+[self.outlier_f2],
                                       self.matches_with_outlier,
                                       np.eye(3), 1)
    ...
    def test_leastSquaresFit(self):
        M = alignment.leastSquaresFit(self.f1+[self.outlier_f1],
                                      self.f2+[self.outlier_f2],
                                      self.matches_with_outlier,
                                      1,
                                      [0, 1, 2, 3])
...
class TestBlend(unittest.TestCase):
    ...
    def test_imageBoundingBox(self):
        ...
        sol_minX, sol_minY, sol_maxX, sol_maxY = int(5-9*np.sin(np.pi/4)), int(-5), int(5+9*np.sin(np.pi/4)), int(18*np.sin(np.pi/4)-5)
        self.assertAlmostEqual(minX, sol_minX, msg='Expected bounding box min x to be {} +/-1 but got {}.'.format(sol_minX, minX), delta=1.01,)
        self.assertAlmostEqual(maxY, sol_maxY, msg='Expected bounding box max y to be {} +/-1 but got {}.'.format(sol_maxY, maxY), delta=1.01,)
        self.assertAlmostEqual(maxX, sol_maxX, msg='Expected bounding box max x to be {} +/-1 but got {}.'.format(sol_maxX, maxX), delta=1.01,)
        self.assertAlmostEqual(minY, sol_minY, msg='Expected bounding box min y to be {} +/-1 but got {}.'.format(sol_minY, minY), delta=1.01,)

    def test_getAccSize(self):
        ipv = [blend.ImageInfo("test1", self.testimage, self.rot_trans_transform1), blend.ImageInfo("test2", self.testimage, self.rot_trans_transform2)]
        accWidth, accHeight, channels, width, translation = blend.getAccSize(ipv)
        self.assertAlmostEqual(accWidth, 20, msg='Expected acc width to be {} +/-1 but got {}.'.format(20, accWidth), delta=1.01,)
        self.assertAlmostEqual(accHeight, 20, msg='Expected acc height to be {} +/-1 but got {}.'.format(20, accHeight), delta=1.01,)
