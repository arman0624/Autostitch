...
def warpLocal(src, uv):
    ...
    Input:
        ...
        uv --     warped image in terms of addresses of each pixel in the
...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    # Applies the spherical correction, i.e.,
    # computes the Euclidean coordinates,
    # and projects the point to the z=1 plane at (xt/zt,yt/zt,1),
    # then distorts with radial distortion coefficients k1 and k2
    # xf, yf are inputs and are used to compute xt, yt
    # as the output. They all have the shape
    ...
    yt = y / z
    r2 = xt**2 + yt**2
    radial_distortion = one + k1 * r2 + k2 * r2**2
    xt *= radial_distortion
    yt *= radial_distortion
...
def warpSpherical(image, focalLength, k1=-0.21, k2=0.26):
    ...
    # computes spherical warp
    # computes the addresses of each pixel of the output image in the
    # source image
    uv = computeSphericalWarpMappings(np.array(image.shape), focalLength, k1, k2)

    # warps image based on backwards coordinates
